...
import jieba
j1 = jieba.Tokenizer(dictionary='jieba_coarse.txt')
j2 = jieba.Tokenizer(dictionary='jieba_fine.txt')
...
def cut(x):
    f = lambda x: x.strip() != ''
    ans = [list(filter(f, list(j1.cut(x)))), list(filter(f, list(j2.cut(x))))]
    return ans
